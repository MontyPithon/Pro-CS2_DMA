...
void Cheats::Run()
{
	...
	for (int i = 0; i < EntityList.size(); i++)
	{
		...
		// Check visibility for ESP rendering
		bool isVisible = !MenuConfig::VisibleCheck || Entity.IsVisibleTo(LocalEntityPlayer.LocalPlayerControllerIndex);
		...
		// Only render ESP if player is visible (when visibility check is enabled)
		if (isVisible)
		{
			// Draw Bone
